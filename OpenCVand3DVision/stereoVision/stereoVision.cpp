...
int main(int argc, char **argv){
    ...
    // change v++ and u++ to v+=2 and u+=2 if your machine is slow for getting a sparser cloud
    for(int v=0; v<left.rows; v++)
    for(int u=0; u<left.cols; u++){
        if(disparity.at<float>(v,u) <= 10.0 || disparity.at<float>(v,u) >= 96.0)
            continue;
        
        Vector4d point(0,0,0, left.at<uchar>(v,u) / 255.0); // the first 3 arguments are xyz, the 4th
                                                            // is color
...
void showPointCloud(vector<Vector4d, Eigen::aligned_allocator<Vector4d>> &pointcloud){
    if(pointcloud.empty()){
        cerr << "Point Cloud is Empyt." << endl;
        return;
    }
