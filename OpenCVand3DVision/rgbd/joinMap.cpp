...
int main(int argc, char **argv) {
    ...
    for (int i = 0; i < 5; i++) {
        ...
        // getting the camera poses from poses.txt which is of the form
        // x, y, z, qx, qy, qz, qw here, qw is the real part of quaternion 
        double data[7] = {0};
        for (auto &d:data)
            fin >> d;
    ...
    // compute point cloud using camera intrinsics 
    double cx = 325.5;
    double cy = 253.5;
    double fx = 518.0;
    double fy = 519.0;
    ...
    for (int i = 0; i < 5; i++) {
        ...
        for (int v = 0; v < color.rows; v++)
            for (int u = 0; u < color.cols; u++) {
                ...
                // Transforming the point's coordinate from Camera's to World's
                // using Camera's pose 'T', which is in world coordinates 
